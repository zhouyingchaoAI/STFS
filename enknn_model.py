    ...
    def _calculate_last_year_offset_prediction(self, line_data: pd.DataFrame, 
        ...
        try:
            ...
            for i in range(days):
                ...
                pred_date = (datetime.strptime(predict_start_date, "%Y%m%d") + timedelta(days=i)).strftime("%Y%m%d")
                ...
                # 判断是否节假日
                is_holiday = (pred_day_features == DAYTYPE_DICT['holiday'])
                # 偏移量调整
                offset = overall_offset_per_day
                if is_holiday and offset != 0:
                    offset = offset + abs(offset) * 2.2  # 节假日偏移上浮20%（取绝对值）
    ...
    def diagnose_zero_predictions(self, line_data: pd.DataFrame, line_no: str, 
        ...
        try:
            ...
            diagnosis['recommendations'].extend([
                ...
                "尝试不同的K值参数和权重配置",
                "考虑特征工程或数据预处理",
                f"当前算法权重配置: KNN={weights.get('knn', 0.6)}, 去年同期+偏移={weights.get('last_year_offset', 0.4)}"
            ])
        except Exception as e:
