...
def plot_hourly_predictions(
    ...
    并增加"去年同期+整体偏移"与"历史当小时客流"的误差率曲线
    ...
    参数:
        ...
        show_pred: 是否显示预测曲线
        show_pred_error: 是否显示预测误差率曲线
        show_last_year: 是否显示去年同期曲线
        show_last_year_offset: 是否显示去年同期+整体偏移曲线
        show_last_year_offset_error: 是否显示去年同期+整体偏移误差率曲线
    """
    # 配置字体和格式
    configure_fonts()
    ...
    for idx, line in enumerate(lines):
        ...
        try:
            ...
            for h in hours:
                ...
                if not np.isnan(hist_val) and not np.isnan(pred_val) and hist_val != 0:
                    error = abs(hist_val - pred_val) / abs(hist_val) * 100
                else:
                    error = np.nan
                pred_errors.append(error)

            # 6. 计算去年同期+整体偏移误差率
            last_year_plus_offset_errors = []
            for h in hours:
                hist_val = hist_hour_flow.get(h, np.nan)
                ...
                if not np.isnan(hist_val) and not np.isnan(offset_val) and hist_val != 0:
                    error = abs(hist_val - offset_val) / abs(hist_val) * 100
                else:
                    error = np.nan
                last_year_plus_offset_errors.append(error)
        ...
        # 误差率曲线（右侧y轴）
        has_pred_error = show_pred_error and any([not np.isnan(e) for e in pred_errors])
        has_offset_error = show_last_year_offset_error and any([not np.isnan(e) for e in last_year_plus_offset_errors])

        # 计算中间60%平均准确率（仅统计7点到23点）
        mean_acc_text = None
        if has_pred_error:
            # 只取7点到23点
            start_hour = 7
            end_hour = 23
            error_range = range(start_hour, end_hour + 1)
            valid_errors = [pred_errors[h] for h in error_range if not np.isnan(pred_errors[h])]
            if len(valid_errors) > 0:
                ...
                if upper_idx <= lower_idx:
                    middle_errors = sorted_errors
                else:
                    middle_errors = sorted_errors[lower_idx:upper_idx]
                if len(middle_errors) > 0:
                    mean_error = np.mean(middle_errors)
                    mean_acc = 100 - mean_error
                    mean_acc_text = f"中间60%平均准确率(7-23点): {mean_acc:.2f}%"

        lines_right = []
        labels_right = []
        if has_pred_error or has_offset_error:
            ax2 = ax.twinx()
            ax2.ticklabel_format(style='plain', axis='y')
            
            if has_pred_error:
                line5, = ax2.plot(hours, pred_errors, marker='s', linestyle='--', 
                                color=color_map['相对误差百分比'], label='相对误差(%)')
                lines_right.append(line5)
                labels_right.append(f'相对误差(%)（{color_map["相对误差百分比"]}）')
                ...
                if mean_acc_text is not None:
                    ax2.text(0.98, 0.98, mean_acc_text,
                           ...
                           color=color_map['相对误差百分比'])
            
            if has_offset_error:
                line6, = ax2.plot(hours, last_year_plus_offset_errors, marker='x', linestyle='--', 
                                color=color_map['去年同期+偏移误差率'], label='去年同期+偏移误差率(%)')
                lines_right.append(line6)
                labels_right.append(f'去年同期+偏移误差率(%)（{color_map["去年同期+偏移误差率"]}）')
            
            # 设置右侧y轴标签
            if has_pred_error and has_offset_error:
                ax2.set_ylabel("误差率(%)", fontsize=12, 
                             fontproperties=my_font if my_font is not None else None,
                             color=color_map['相对误差百分比'])
            elif has_pred_error:
                ax2.set_ylabel("相对误差(%)", fontsize=12,
                             fontproperties=my_font if my_font is not None else None,
                             color=color_map['相对误差百分比'])
            elif has_offset_error:
                ax2.set_ylabel("去年同期+偏移误差率(%)", fontsize=12,
                             fontproperties=my_font if my_font is not None else None,
                             color=color_map['去年同期+偏移误差率'])
            
            ax2.tick_params(axis='y', labelcolor=color_map['相对误差百分比'])
    ...
    # 设置总标题并保存
    plt.suptitle(f"小时客流量预测 (预测日: {predict_date})", fontsize=16, 
                fontproperties=my_font if my_font is not None else None)
...
def plot_daily_predictions(
    ...
    for idx, line in enumerate(lines):
            ...
            for i, d in enumerate(pred_dates_str):
                try:
                    ...
                    if d in [dt.strftime('%Y%m%d') for dt in curr_hist_dates]:
                        ...
                        if hist_val != 0:
                            abs_perc_error = abs(hist_val - pred_val) / abs(hist_val) * 100
                        else:
        ...
        # 误差率曲线
        has_pred_error = show_pred_error and any([not np.isnan(e) for e in abs_perc_errors])
        has_offset_error = show_last_year_offset_error and any([not np.isnan(e) for e in last_year_plus_offset_errors]) if 'last_year_plus_offset_errors' in locals() else False

        # 计算中间60%平均准确率
        mean_acc_text = None
        if has_pred_error:
            # 只取非nan的误差
            valid_errors = [e for e in abs_perc_errors if not np.isnan(e)]
            if len(valid_errors) > 0:
                sorted_errors = sorted(valid_errors)
                n = len(sorted_errors)
                lower_idx = int(n * 0.2)
                ...
                if len(middle_errors) > 0:
                    mean_error = np.mean(middle_errors)
                    mean_acc = 100 - mean_error
                    mean_acc_text = f"中间60%平均准确率: {mean_acc:.2f}%"
                else:
                    mean_acc_text = None
            else:
                mean_acc_text = None
        ...
        if has_pred_error or has_offset_error:
            ...
            ax2.ticklabel_format(style='plain', axis='y')
            if has_pred_error:
                line3, = ax2.plot([dt.strftime('%Y-%m-%d') for dt in pred_dates], abs_perc_errors, marker='s', linestyle='--', color=color_map['相对误差百分比'], label='相对误差(%)')
                ...
                line3 = None
            if has_offset_error:
                line5, = ax2.plot([dt.strftime('%Y-%m-%d') for dt in pred_dates], last_year_plus_offset_errors, marker='x', linestyle='--', color=color_map['去年同期+偏移误差率'], label='去年同期+偏移误差率(%)')
                lines_right.append(line5)
                labels_right.append(f'去年同期+偏移误差率(%)（{color_map["去年同期+偏移误差率"]}）')
            else:
                line5 = None
            # 设置y轴标签
            if has_pred_error and has_offset_error:
                ax2.set_ylabel("误差率(%)", fontsize=12, fontproperties=my_font if my_font is not None else None, color=color_map['相对误差百分比'])
            elif has_pred_error:
                ax2.set_ylabel("相对误差(%)", fontsize=12, fontproperties=my_font if my_font is not None else None, color=color_map['相对误差百分比'])
            elif has_offset_error:
                ax2.set_ylabel("去年同期+偏移误差率(%)", fontsize=12, fontproperties=my_font if my_font is not None else None, color=color_map['去年同期+偏移误差率'])
            ax2.tick_params(axis='y', labelcolor=color_map['相对误差百分比'])
            # 合并图例，显示颜色含义
            handles = line_handles + lines_right
            labels = line_labels + labels_right
